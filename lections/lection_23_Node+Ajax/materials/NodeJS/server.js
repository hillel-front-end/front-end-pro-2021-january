...
app.get('/candidates', function(request, response) {
  const {maxAge = 100, minAge = 1} = request.query;

  fs.readFile('./models/data.json', 'utf-8', function(error, payload) {

    const parsedPayload = JSON.parse(payload);
    const filteredCaindidates =  parsedPayload.filter(item => item.age >= minAge && item.age <= maxAge);
...
// http://localhost:3003/candidate/id

app.get('/candidate/:id', function(request, response) {
  ...
  fs.readFile('./models/data.json', 'utf-8', function(error, payload) {
    ...
    const parsedPayload = JSON.parse(payload);
    const filteredCaindidates =  parsedPayload.filter(item => item.id == id);

    if (filteredCaindidates.length == 0) {
      response.status(404);
      response.json({ message: 'Cadidate Not found' });
      return;
    }
